...
void DrawLineAcc(int x1, int y1, int length, char* data){
	int i = 0;
	WAIT_FOR_GRAPHICS;
	GraphicsX1Reg = x1;    // write coords to x1, y1
	GraphicsY1Reg = y1;
	GraphicsX2Reg = x1 + length - 2;
	GraphicsCommandReg = DrawHLineAcc;
	while (i < length){
		GraphicsColourReg = data[i++];
	}
